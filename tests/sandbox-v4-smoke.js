...
(async()=>{
  ...
  const uncertainty=await page.locator('#tkBody').innerText();
  for(const term of ['Gradient uncertainty','Repeated sets','Power rule'])if(!uncertainty.includes(term))throw new Error(`Uncertainty engine missing ${term}`);
