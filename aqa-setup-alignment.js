...
function apparatusReference(id){
  ...
  return '<details class="aqa-render practical3d-details"'+((id===2||id===4)?' open':'')+'><summary>Explore full 3D apparatus · '+modeName+'</summary><div id="'+hostId+'" class="young3d practical3d" data-practical-id="'+id+'" data-mode="'+currentMode+'"><canvas aria-label="Rotatable 3D model of '+(r.alt||current.title)+'"></canvas><div class="young3d-fallback" hidden>'+fallback+'</div><div class="young3d-controls"><span class="young3d-status" role="status">Loading full 3D model…</span><button type="button" data-young-reset>Reset view</button><button type="button" data-young-expand>Enlarge 3D view</button><a class="practical3d-download" href="'+cfg.file+'" download>Download GLB</a></div></div>'+notes+'<p>Interactive model for learning the apparatus layout. Use your teacher-approved real-lab arrangement for practical work.</p></details>';
}
