...
function ensurePanel(){
  if(!current)return;
  const readouts=document.querySelector('#readouts');if(!readouts)return;
  let root=panel();
  if(!root){
    root=document.createElement('section');root.id='sandboxLiveScope';root.className='sandbox-scope';root.innerHTML=`
...
function sample(detail){
  ensurePanel();if(!current||frozen||!detail?.running)return;
  const now=performance.now();if(now-lastSample<90)return;lastSample=now;
  refreshFields();const reading=numericReadings().find(x=>x.name===selectedField);if(!reading)return;
...
function draw(latest){
  const root=panel();if(!root||!current)return;
  refreshFields();const arr=seriesFor(keyFor(),selectedField),canvas=root.querySelector('#scopeCanvas'),ctx=canvas.getContext('2d');
...
document.addEventListener('practicallab:runstate',()=>{ensurePanel();draw();});
