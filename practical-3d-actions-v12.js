...
function actionsFor(id,mode){
  switch(Number(id)){
    ...
    case 11:return[
      action('Rotate coil 90°','Rotate the search coil to compare orientation to the field.',['search coil','coil'],async(api,name)=>{const next=(api.angleOf(name)||0)+Math.PI/2;await api.animateGroup(name,{angle:next,axis:[0,0,1],pivot:[.05,-.04,1.80]},650,'smooth');status(api,'Search coil rotated 90° · consider how flux linkage depends on orientation');}),
...
window.__practical3DPhysicalActionsV12={version:'14.0',actionsFor};window.__practical3DPhysicalActionsV14=window.__practical3DPhysicalActionsV12;
})();